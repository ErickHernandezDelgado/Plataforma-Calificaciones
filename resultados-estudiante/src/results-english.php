...
foreach ($students as $student):
    ...
    // CAMBIO CLAVE: Agregar filtro Language = 'en' para obtener solo materias de inglés
    // Consulta calificaciones por estudiante y materia para los 3 términos (primaria)
    $sql = "SELECT subj.SubjectName, subj.id AS SubjectId,
            ...
            WHERE r.StudentId = :studentId AND subj.Language = 'en'
            GROUP BY subj.id, subj.SubjectName
            ORDER BY subj.SubjectName";
    $query = $dbh->prepare($sql);
...
<div class="boleta">
    ...
    <div class="header">
        ...
        <h3>ENGLISH GRADES REPORT / BOLETA DE CALIFICACIONES INGLÉS</h3>
    </div>

    <table class="student-info">
        <tr>
            <td><b>Student Name / Nombre del Estudiante:</b> <?= htmlentities($student['StudentName']) ?></td>
            <td><b>Grade / Grado:</b> <?= htmlentities($student['ClassName']) ?></td>
        </tr>
        <tr>
            <td><b>Section / Sección:</b> <?= htmlentities($student['Section']) ?></td>
            <td><b>School Year / Ciclo Escolar:</b> 2025</td>
        </tr>
        <tr>
            <td><b>Shift / Turno:</b> Morning / Matutino</td>
    ...
    <table class="grades">
        <thead>
            <tr>
                <th>#</th>
                <th>English Subject / Materia de Inglés</th>
                <th>1st Term<br>(Período I)</th>
                <th>2nd Term<br>(Período II)</th>
                <th>3rd Term<br>(Período III)</th>
                <th>Final Average<br>Promedio Final</th>
            </tr>
        </thead>
        <tbody>
            <?php
            if (count($results) > 0) {
                ...
                echo "<tr><td colspan='6' style='text-align:center; padding: 20px;'><em>No English grades available / Sin calificaciones de inglés disponibles</em></td></tr>";
            }
            ?>
        </tbody>
    </table>
    ...
    <table class="footer-table">
        <tr>
            <td>_________________________<br>English Teacher's Signature<br>Firma del Maestro de Inglés</td>
            <td>_________________________<br>Principal's Signature<br>Firma del Director</td>
        </tr>
        <tr>
            <td>_________________________<br>Parent/Guardian Signature<br>Firma del Padre, Madre o Tutor</td>
            <td>Tampico, Tamaulipas<br>_________________________<br>Date / Fecha</td>
        </tr>
        <tr>
            <td colspan="2" style="text-align: center;"><i>*This is a provisional document without official validity.<br>*Este es un documento provisional sin validez oficial.</i></td>
        </tr>
    </table>
</div>
...
$dompdf->stream("English_Grades_{$classId}.pdf", ["Attachment" => false]);
exit;
?>
